...
datos_aero_mendoza <- read_excel("187309 - Mendoza Aero (87418) .xlsx")

# Primero, transformo el formato de los datos del data frame a los tipos que corresponden.

names(datos_aero_mendoza)[6] <- "vto_max_veloc" 

datos_aero_mendoza$fecha <- as.Date(datos_aero_mendoza$fecha)
datos_aero_mendoza$tmax <- as.double(datos_aero_mendoza$tmax)
datos_aero_mendoza$hr <- as.double(datos_aero_mendoza$hr)
datos_aero_mendoza$vto_max_veloc <- as.double(datos_aero_mendoza$vto_max_veloc)
...
datos_aero_mendoza <- datos_aero_mendoza %>%
  mutate(month = month(fecha))

skim_without_charts(datos_aero_mendoza)
...
hr_ridges_mendoza <- ggplot(datos_aero_mendoza) +
  ...
  labs(title = 'Distribución de Humedades relativas mensuales en Mendoza',
       subtitle = "Distribución de registro de humedades relativas medias de Mendoza desde 1961 a 2021",
       x = "Humedad Relativa %",
       y = "Mes") +
  theme_ridges() +
  theme(
...
vto_max_veloc_ridges_mendoza <- ggplot(datos_aero_mendoza) +
  ...
  labs(title = 'Distribución de Velocidades máximas de viento en Mendoza',
       subtitle = "Distribución de registro de velocidades de viento máximas de Mendoza desde 1961 a 2021",
...
tmax_ridges_mendoza <- ggplot(datos_aero_mendoza) +
  ...
  labs(title = 'Distribución de Temperaturas máximas mensuales en Mendoza',
       subtitle = "Distribución de registro de temperaturas máximas de Mendoza desde 1961 a 2021",
...
plot(tmax_ridges_mendoza)
plot(hr_ridges_mendoza)
plot(vto_max_veloc_ridges_mendoza)
...
datos_aero_mendoza_mensual <- datos_aero_mendoza %>%
  drop_na() %>%
  filter(vto_max_veloc < 60) %>%  # este filtrado lo realizo debido a que parece haber errores en la medicion que tenian velocidades del viento superiores a 400 km/h
  group_by(month) %>%
...
datos_aero_mendoza_simulados <- data.frame()

for (i in 1:12){
  temperaturas <- rnorm(runs, datos_aero_mendoza_mensual[[i,2]], datos_aero_mendoza_mensual[[i,3]])
  vientos <- rnorm(runs, datos_aero_mendoza_mensual[[i,6]], datos_aero_mendoza_mensual[[i,7]])
  humedades_relativas <- rnorm(runs, datos_aero_mendoza_mensual[[i,4]], datos_aero_mendoza_mensual[[i,5]])
  ...
  datos_aero_mendoza_simulados <- rbind(datos_aero_mendoza_simulados, simulaciones)
  i = i + 1
}

tasas_de_evaporacion_ridges_mendoza <- ggplot(datos_aero_mendoza_simulados) +
  ...
  labs(title = 'Distribución de tasas de evaporación mensuales en Mendoza',
       subtitle = "Distribución de simulación de tasas de evaporación para Mendoza a partir de datos de clima de 1961
...
plot(tasas_de_evaporacion_ridges_mendoza)

# Ahora obtendré el promedio y el desvio mensual de las distribuciones 
# definidas en la simulación.

datos_aero_mendoza_mensual_simulacion <- datos_aero_mendoza_simulados %>%
  drop_na()%>%
  group_by(mes)%>%
...
datos_aero_mendoza_mensual_simulacion <- datos_aero_mendoza_mensual_simulacion %>%
...
names(datos_aero_mendoza_mensual_simulacion)[1] <- "month"

probabilidades_mensuales_tasa_0.5_mendoza <- ggplot(datos_aero_mendoza_mensual_simulacion)+
...
plot(probabilidades_mensuales_tasa_0.5_mendoza)
...
datos_aero_mendoza_simulados_T20 <- data.frame()
...
for (i in 1:12){
  ...
  simulaciones <- data.frame(mes,
                             ...
                             humedades_relativas,
                             tasa_de_evaporacion_simulada)
  datos_aero_mendoza_simulados_T20 <- rbind(datos_aero_mendoza_simulados_T20, simulaciones)
  i = i + 1
}

datos_aero_mendoza_simulados_T25 <- data.frame()
...
for (i in 1:12){
  ...
  simulaciones <- data.frame(mes,
                             ...
                             humedades_relativas,
                             tasa_de_evaporacion_simulada)
  datos_aero_mendoza_simulados_T25 <- rbind(datos_aero_mendoza_simulados_T25, simulaciones)
  i = i + 1
}

datos_aero_mendoza_simulados_T30 <- data.frame()
...
for (i in 1:12){
  ...
  simulaciones <- data.frame(mes,
                             ...
                             humedades_relativas,
                             tasa_de_evaporacion_simulada)
  datos_aero_mendoza_simulados_T30 <- rbind(datos_aero_mendoza_simulados_T30, simulaciones)
  i = i + 1
}

datos_aero_mendoza_simulados_T35 <- data.frame()
...
for (i in 1:12){
  ...
  simulaciones <- data.frame(mes,
                             ...
                             humedades_relativas,
                             tasa_de_evaporacion_simulada)
  datos_aero_mendoza_simulados_T35 <- rbind(datos_aero_mendoza_simulados_T35, simulaciones)
  i = i + 1
}
...
datos_aero_mendoza_mensual_simulacion_T20 <- datos_aero_mendoza_simulados_T20 %>%
  drop_na()%>%
  group_by(mes)%>%
...
datos_aero_mendoza_mensual_simulacion_T25 <- datos_aero_mendoza_simulados_T25 %>%
  drop_na()%>%
  group_by(mes)%>%
...
datos_aero_mendoza_mensual_simulacion_T30 <- datos_aero_mendoza_simulados_T30 %>%
  drop_na()%>%
  group_by(mes)%>%
...
datos_aero_mendoza_mensual_simulacion_T35 <- datos_aero_mendoza_simulados_T35 %>%
  drop_na()%>%
  group_by(mes)%>%
...
names(datos_aero_mendoza_mensual_simulacion_T20)[1] <- "month"
names(datos_aero_mendoza_mensual_simulacion_T25)[1] <- "month"
names(datos_aero_mendoza_mensual_simulacion_T30)[1] <- "month"
names(datos_aero_mendoza_mensual_simulacion_T35)[1] <- "month"

# Ahora pondré todos los resultados de las simulaciones en un único Dataframe

Simulaciones_finales_mendoza <- merge(datos_aero_mendoza_mensual_simulacion, datos_aero_mendoza_mensual_simulacion_T20, by = "month")
Simulaciones_finales_mendoza <- merge(Simulaciones_finales_mendoza, datos_aero_mendoza_mensual_simulacion_T25, by = "month")
Simulaciones_finales_mendoza <- merge(Simulaciones_finales_mendoza, datos_aero_mendoza_mensual_simulacion_T30, by = "month")
Simulaciones_finales_mendoza <- merge(Simulaciones_finales_mendoza, datos_aero_mendoza_mensual_simulacion_T35, by = "month")

# Ahora realizaré el gráfico de probabilidades para todas las temperaturas
#######
#######

probabilidades_mensuales_T35_mendoza <- ggplot(Simulaciones_finales)+
...
plot(probabilidades_mensuales_T35_mendoza)

tasas_de_evaporacion_ridges_T35_mendoza <- ggplot(datos_aero_mendoza_simulados_T35) +
  geom_density_ridges_gradient(scale = 3.5,
                               rel_min_height = 0.01,
                               aes(x = tasa_de_evaporacion_simulada, y = as.factor(mes),
                                   fill = ..x..)) +
...
plot(tasas_de_evaporacion_ridges_T35_mendoza)

#######
#######

probabilidades_mensuales_T30_mendoza <- ggplot(Simulaciones_finales)+
...
plot(probabilidades_mensuales_T30_mendoza)

# position=position_dodge(width=0.9), hjust=0.75

tasas_de_evaporacion_ridges_T30_mendoza <- ggplot(datos_aero_mendoza_simulados_T30) +
  geom_density_ridges_gradient(scale = 3.5,
                               rel_min_height = 0.01,
                               aes(x = tasa_de_evaporacion_simulada, y = as.factor(mes),
                                   fill = ..x..)) +
...
plot(tasas_de_evaporacion_ridges_T30_mendoza)
